...
struct Panel {
    ...
    bits: Vec<Vec<u64>>,
    hap_names: Vec<String>,
    sample_of_hap: Vec<usize>,
    sample_pop: Vec<String>,
    sample_super: Vec<String>,
}

fn load(dir: &str) -> anyhow::Result<Panel> {
    // --- samples ---------------------------------------------------------
...
#[inline]
fn arc_diffs(hap: &[u64], arc: &Archaic, lo: usize, hi: usize) -> (u32, u32) {
...
#[inline]
fn diffs(a: &[u64], b: &[u64], lo: usize, hi: usize) -> u32 {
    let (w0, w1) = (lo / 64, hi.div_ceil(64));
    let mut n = 0u32;
    for w in w0..w1 {
        let mut x = a[w] ^ b[w];
